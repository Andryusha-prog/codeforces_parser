...
def welcome(message):
    """
    Выводит приветственное сообщение, сообщает о количестве задач, их сложности и темах, а так же отрисовывает кнопки выбора действий
    :param message:
    :return:
    """
    ...
    bot.send_message(chat_id,
                     f'Привет! На данный момент в базе хранится {start_db.get_count_data()} задач с сайта codeforces.com. '
...
def select_func(call):
    ...
    bot.send_message(chat_id, f'Введите тему задачи:')
...
def input_level(message):
    """
    Функция обработки введенной сложности задач и вывода задач, соответствующих введенным данным
    :param message:
    :return:
    """
    level = message.text
