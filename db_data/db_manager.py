...
class ManagerDB(StartDB):
    ...
    def insert_problems(self, problems_list: list[dict], tag_words: dict):
        ...
        with self.session as session:
            ...
            for problems in problems_list:
                ...
                #проверка rating нужна, что бы не происходила ошибка при отсутствии данных параметра во входных данных
    ...
    def insert_tag_words(self, translate_words: dict[str, str]):
        ...
        :param translate_words: словарь с оригинальным и переведенным списком тем
        :return:
        """
    ...
    def select_problem_rating(self) -> dict[str, int]:
        """
        Возвращает словарь с минимальным и максимальным значением уровня сложности задачи
        :return:
        """
        min_max_rate = {}
        with self.session as session:
    ...
    def result_select(self, input_data: dict[str, int]) -> list:
        """
        Выполняет выборку данных по теме и уровню сложности, введенном пользователем
        :param input_data: список множеств состоящий из данных соединенных таблиц
        :return:
        """
        result_list = []
        result_list_contest = []
